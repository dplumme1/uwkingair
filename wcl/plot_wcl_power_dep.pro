...
pro plot_wcl_power_dep, p1r2, p2r2,rgalt0,rtime,h, dir,fname

power=p1r2
power2=p2r2
rgalt=rgalt0

;We should now have all the lidar data and a/c data for NetCDF file writing of current chunk
;next setup for plots
minalt=min(rGalt[where(rgalt gt 0)]) ; find the lowest altitude of the airplane in this chunk 
id=where(h/1000 gt 0 and h/1000 lt 4,nbindis)     ; lidar display range, 
power  = power[*,id]
power2  = power2[*,id]
 
y =(minalt+indgen(nbindis)*1.5)/1000 ;set the ordinate (height, 3.75 is the range reslution of the lidar)
dep   = power2/power/3.67589/4.5 


;shift data for plots
FOR j=0,N_ELEMENTS(rtime)-1 DO BEGIN
  ;IF (rGALT[j] GT 0) THEN shftalt = ROUND((rgalt[j]-minalt)/3.75) else shftalt=1;if galt is bad, use last value
  IF (rGALT[j] GT 0) THEN shftalt = ROUND((rgalt[j]-minalt)/1.5) else shftalt=1;if galt is bad, use last value

  if ((shftalt ge 1) and (shftalt le nbindis)) then begin
    dep[j,*] = SHIFT(dep[j,*],0,shftalt)
    power[j,*] = SHIFT(power[j,*],0,shftalt)
    dep[j,0:shftalt-1]= 9999  ;incase of rollover
    power[j,0:shftalt-1]= 9999  ;incase of rollover  
  endif
ENDFOR


x =rtime; set the abscissa (time)
;set the label of the time axis
pnumber=N_ELEMENTS(x)
fnumber=round(pnumber/5.0)
xticks=5
XLAB=STRARR(xticks+1)
FOR j=0, 5 DO BEGIN
  if (j*fnumber  lt pnumber-1) then begin
    hr = LONG(x[j*fnumber]/3600L) & mn = round((x[j*fnumber]-(hr*3600L))/60)
  ENDIF ELSE begin
    hr = LONG(x[pnumber-1]/3600L) & mn = round((x[pnumber-1]-(hr*3600L))/60)
  endelse
  
  
  
  hr = hr MOD 24
  IF (mn eq 60)THEN begin
    hr=hr+1
    mn=0
  endif

  IF (hr EQ 0) THEN hr = '00' ELSE hr = STRTRIM(STRING(hr),2)
  IF (LONG(hr) LT 10 AND LONG(hr) GT 0) THEN hr = '0'+hr
  IF (mn EQ 0) THEN mn = '00' ELSE mn = STRTRIM(STRING(mn),2)
  IF (LONG(mn) LT 10 AND LONG(mn) GT 0) THEN mn = '0'+mn
...
set_plot,'x'
window,0,xsize=800,ysize=500,TITLE='LIDAR_image',/pixmap
!x.ticklen=0.03
!y.ticklen=-0.02
!x.minor=5
!p.charsize=1.
...
ERASE, color24([255,255,255])
colorliu, lutr,lutg, lutb
;setup for plots
x_title='!5UTC (hh:mm:ss)' & y_title='!5Altitude (km)'
xtickname=['','','','','','','','']
ytickname=['','','','','','','','']
 
 
;plot depolarization
cb_title='!5Depolarization (uncal)'
image_pos=[0.07,0.09,0.87,.45]
cb_pos=[0.97,0.10,0.99,0.40]
max_histo=0.7 & min_histo=0 ;-20
image_title = ' '
dep=temporary(dep)
plot_gif_tv,image_title,image_pos,cb_pos,cb_title,dep,x,Y,x_title,max_histo, min_histo,y_title,xlab,ytickname, xticks


;plot co-polar power
image_pos=[0.07,0.54,0.87,.90]
cb_pos=[0.97,0.60,0.99,0.90]
cb_title='!510Log10(Power)'
max_histo=0 & min_histo=-50  
ind = WHERE(power GT -32767)
ind = WHERE(power GT 0)
power[ind] = 10*ALOG10(power[ind]); show the logarithmic power
plot_gif_tv,image_title,image_pos,cb_pos,cb_title,power,x,Y,x_title,max_histo, min_histo,y_title,xlab,ytickname, xticks

 
;save the image file
fnamew=dir+fname 
 null = TVREAD(filename=fnamew, /gif, /nodialog)

end

; Subroutine used in this plotting code.
; COLOR24
; COLORLIU
; PLOT_GIF_TV.
; TVIMAGE.
...
PRO colorliu, LUTR, LUTG, LUTB, mmin, mmax
...
;modified by  dliu3@uwyo.edu
LUTR[250:254]=LUTR[250] & LUTG[250:254]=LUTG[250] & LUTB[250:254]=LUTB[250]



;;//
if N_ELEMENTS(mmin) GT 0 AND N_ELEMENTS(mmax) GT 0 then begin

  ;print, mmax, mmin
  len = mmax - mmin
  index = mmin + indgen(256) * len / 256.

  ind = where(index GE 0, cnt)

  mid = ind[0] -1
  ;print, mid

  LUTR[mid-1:mid+1]= 255
  LUTG[mid-1:mid+1]= 255
  LUTB[mid-1:mid+1]= 255
...
pro  plot_gif_tv,image_title,image_pos,cb_pos,cb_title,temp_array,x,y,x_title,max_histo, min_histo,y_title,xtick,ytick,xticks

  ;An example to call this routine
  ;image_title='  '
  ;image_pos=[0.2,0.65,0.75,.95]
  ;cb_pos=[0.85,0.65,0.9,0.94]
  ;cb_title='Scattering Ratio'
  ;x=time_offset/3600.
  ;y=height ;km,AGL
  ;temp_array=transpose(ASR) ;zz
  ;x_title='Time, UTC'
  ;y_title='Height (km, AGL)'
  ;max_histo=5.
  ;min_histo=0.
  ;plot_gif,image_title,image_pos,cb_pos,cb_title,temp_array,x,y,x_title,max_histo, min_histo,y_title

  ;min(temp_array)
  aa=size(x) & ix=aa[1]
  image=bytscl(temp_array,min=min_histo,max=max_histo)

  contour, float(temp_array), x, y, xstyle=1, $
    ystyle=1, title=image_title, xtitle=x_title, $
    ytitle=y_title, position=image_pos, $
    /nodata,xticks=xticks, $
    background=!d.n_colors-1 , color=0 ,/noerase,xtickname=xtick,xrange=[x[0],x[ix-1]],ytickname=ytick;,yticklen=-0.02
  ...
  ;tv, congrid(image,sx,sy), px[0], py[0];,order=1
  tvimage, image, POSITION=image_pos,/NOINTERPOLATION

  contour, float(temp_array), x, y, xstyle=1, $
    ystyle=1, title=image_title, xtitle=x_title, $
    ytitle=y_title, position=image_pos, $
    /nodata, /noerase,xticks=xticks, $
    color=0,xtickname=xtick,xrange=[x[0],x[ix-1]],ytickname=ytick;,yticklen=-0.02

  ; plot a color bar
  if cb_pos[0] lt 1 then begin

    min_image=0; min(image)
    ;image(where(image eq 255))=0
    max_image=255;255;max(image)
    ...
    for j=0,9 do cb_arr(*,j)=min_histo+((max_histo-min_histo)/(9.))*j
    ;print,max_histo,min_histo,cb_arr(1,*)
    ...
    contour, float(cb_arr), [1,2], cb_arr(1,*), xstyle=4, $
      ystyle=1,  $
      ytitle=cb_title, position=cb_pos, $
      /nodata, /noerase,$
    ...
    sx=px[1]-px[0]+1
    sy=py[1]-py[0]+1
    ;tv, congrid(cb_img,sx,sy), px[0], py[0]
    tvimage, congrid(cb_img,sx,sy), POSITION=cb_pos
    ;print,max(cb_arr(1,*)),max_histo
    contour, float(cb_arr), [1,2], cb_arr(1,*), xstyle=4, $
      ystyle=1,   $
      ytitle=cb_title, position=cb_pos, $
      /nodata, /noerase,$
      background=!d.n_colors-1, color=0,yticklen=0.2
